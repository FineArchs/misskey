...
const urls = computed(() => 
	parsed.value
	? extractUrlFromMfm(parsed.value).filter((url) => appearNote.value.renote?.url !== url && appearNote.value.renote?.uri !== url) 
	: null
);
const collapsibleArea = ref(null);
const isLong = computed(() => 
	defaultStore.state.noteCollapsingCondition === 'parse'
	? shouldCollapsed(appearNote.value, urls.value ?? [])
	: false
);
const collapsed = ref(appearNote.value.cw == null && isLong && defaultStore.state.noteCollapsingCondition !== 'size');
